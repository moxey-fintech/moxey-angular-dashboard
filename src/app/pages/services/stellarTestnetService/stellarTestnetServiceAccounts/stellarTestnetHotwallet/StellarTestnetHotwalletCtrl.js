(function () {
    ...
    /** @ngInject */
    function StellarTestnetHotwalletCtrl($scope,localStorageManagement,$http,errorHandler,_,toastr,sharedResources,$state,$window,$location,extensionsHelper,
        ...
        $scope.withdrawalsDisabled = false;
        ...
        $scope.searchHotwalletFees = function(){
            if($scope.feeSubtype){
                $state.go('transactions.history', {searchHotwalletFees: {code: 'TXLM', subtype: $scope.feeSubtype} });
            }
            else{
        ...
        $scope.searchHotwalletWithdrawals = function(){            
            if($scope.withdrawSubtype){
                $state.go('transactions.history', {searchHotwalletFees: {code: 'TXLM', subtype: $scope.withdrawSubtype, status: 'Pending'} });
            }
            else{
        ...
        $scope.goToHotWalletPage = function(){
            $state.go('stellarTestnetServiceAccounts', {view: 'hotwallet'}, {reload: true});
            $scope.hotwalletSetupStep = 4;
        };
        ...
        vm.getHotwalletActive = function (applyFilter) {
            ...
            if(vm.token) {
                $http.get(vm.serviceUrl + 'admin/hotwallet/active/', {
                    ...
                    vm.getStellarTestnetHotwalletSettings();
                }).catch(function (error) {
                    $scope.loadingHotwalletTransactions =  false;
        ...
        $scope.openSendFromHotwalletModal = function (page, size) {
            vm.theModal = $uibModal.open({
                ...
                templateUrl: page,
                size: size,
                controller: 'SendStellarTestnetHotwalletModalCtrl',
                scope: $scope,
                resolve: {
        ...
        vm.getStellarTestnetHotwalletSettings = function(){
            if(vm.token){
