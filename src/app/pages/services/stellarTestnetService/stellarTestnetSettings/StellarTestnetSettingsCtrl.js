(function () {
    ...
    function StellarTestnetSettingsCtrl($rootScope,$scope,localStorageManagement,$http,$ngConfirm,environmentConfig,$state,extensionsHelper,
                                    $timeout,toastr,$location,errorHandler,Rehive, $uibModal, serializeFiltersService) {
        ...
        $scope.addingFederationAddress = false;
        $scope.federationDomains = [];
        $scope.currentFederationDomain = "--";
        $scope.newFederationDomain = null;
        $scope.downloadingTOMLFile = false;
        ...
        vm.editExtensionSubtypes = $scope.editExtensionSubtypes = {
            ...
            transaction_issue_subtype: null,
        ...
        vm.matchProperty = function(property){
            switch(property){
                ...
                case("transaction_issue_subtype"): return true;
                ...
                buttons: {
                    Add: {
                        ...
                        action: function(scope){
                            scope.deactivateStellarTestnetService();
                        }
                    },
                    close: {
                        text: "Cancel",
        ...
        $scope.deactivateStellarTestnetService = function () {
            if(vm.token) {
        ...
        vm.getFederationdomainAddresses = function(){
            if(vm.token){
                $scope.addingFederationAddress = false;
                $http.get(vm.serviceUrl + 'admin/federation/', {
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': vm.token
                    }
                }).then(function (res) {
                    $scope.currentFederationDomain = res.data.data.domain;
                    $scope.addingFederationAddress = false;
                }).catch(function (error) {
                    $scope.addingFederationAddress = false;
                    if(error.status !== 404){
        ...
        $scope.resetSubtype = function(property){
            ...
            $scope.prevExtensionSubtypes[property] = 'none';
        };

        $scope.addStellarTestnetFederationAddress = function(){
            if(!$scope.newFederationDomain){
                toastr.error('No federation domain provided.');
                return;
            }
            if(vm.token){
                $scope.addingFederationAddress = false;
                $http.post(vm.serviceUrl + 'admin/federation/', {domain: $scope.newFederationDomain}, {
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': vm.token
                    }
                    ...
                    $scope.newFederationDomain = null;
                    $scope.addingFederationAddress = false;
                    vm.getFederationdomainAddresses();
                }).catch(function (error) {
                    $scope.addingFederationAddress = false;
                    errorHandler.evaluateErrors(error.data);
                    errorHandler.handleErrors(error);
        ...
        $scope.updateStellarTestnetWithdrawalSetting = function(){
            if(vm.token){
                $scope.updatingHotwalletSettings = true;
        ...
        $scope.toggleHotwalletWithdrawls = function(){
            ...
            $scope.updateStellarTestnetWithdrawalSetting();
        };
                ...
                $http.get(vm.serviceUrl + 'admin/company/configuration/', {
                    ...
                    for (var property in $scope.defaultConfig) {
                        if ($scope.defaultConfig.hasOwnProperty(property)) {
                            ...
                            $scope.prevExtensionSubtypes[property] = $scope.defaultConfig[property];
        ...
        $scope.setupDefaultSubtypes = function(){
            ...
            // subtypes.push($scope.getPresetSubtype("issue"));
            ...
            vm.editExtensionSubtypes = $scope.prevExtensionSubtypes = {
        ...
        vm.addTransactionSubtype =  function (subtypeObj, last, extensionSubtypes) {
            if (vm.token) {
                ...
                Rehive.admin.subtypes.create(subtypeObj).then(function (res) {
                    if (last) {
                        extensionSubtypes ? vm.updateStellarSubtypes(extensionSubtypes) : vm.getAllSubtypes();
                    }
                    $scope.loadingSubtypes = false;
                    $scope.$apply();
                }, function (error) {
        ...
        vm.updateStellarSubtypes = function(stellarTestnetSubtypes){
            if(vm.token){
                $scope.loadingSubtypes = true;
                $http.patch(vm.serviceUrl + 'admin/company/configuration/', stellarTestnetSubtypes, {
                    headers: {
                        'Content-type': 'application/json',
                        'Authorization': vm.token
                    }
        ...
        $scope.changeSubtype = function(field, type){
            if($scope.editExtensionSubtypes[field].name && $scope.editExtensionSubtypes[field].name !== vm.editExtensionSubtypes[field]){
                ...
                $scope.prevExtensionSubtypes[field] = $scope.editExtensionSubtypes[field];
            }
        ...
        $scope.revertSubtype = function(field, type){
            $scope.editExtensionSubtypes[field] = $scope.prevExtensionSubtypes[field];
            $scope.toggleEditingSubtypes(type);
        };
                    ...
                    if (vm.editExtensionSubtypes.hasOwnProperty(property) && vm.matchProperty(property)) {
                        switch (property) {
                            ...
                            case "transaction_fund_subtype":
                                ...
                                break;
                            case "transaction_issue_subtype":
                                if(vm.editExtensionSubtypes.transaction_issue_subtype !== "hotwallet_deposit"){
                                    $scope.nonDefaultSet = true;
                                }
                                break;
        ...
        $scope.updateDefaultSubtypes = function(){
            ...
            for(var idx = 0; idx < $scope.allSubtypes.length; ++idx){
                ...
                if($scope.allSubtypes[idx].name === "hotwallet_deposit"){
                    extensionSubtypes.transaction_issue_subtype = "hotwallet_deposit";
                }
            ...
            if(!extensionSubtypes.transaction_issue_subtype){
                extensionSubtypes.transaction_issue_subtype = "hotwallet_deposit";
                subtypes.push($scope.getPresetSubtype("fund"));
            }
            ...
            if(subtypes.length > 0){
                ...
                vm.updateStellarSubtypes(extensionSubtypes);
            }
        };
        ...
        $scope.downloadStellarTestnetTOMLFile = function(){
            if(vm.token){
                $scope.downloadingTOMLFile = true;
                $http.get(vm.serviceUrl + 'admin/company/stellar-toml/', {
                    headers: {
                        'Content-type': 'application/json',
                        'Authorization': vm.token
                    }
                }).then(function(res){
                    var tomlLink = res.data.data.file;
                    window.open(tomlLink, '_blank');
                    toastr.success('Stellar TOML file downloaded successfully');
                    $scope.downloadingTOMLFile = false;
                }).catch(function(error){
                    $scope.downloadingTOMLFile = false;
                    errorHandler.evaluateErrors(error.data);
        ...
        vm.fetchServiceUrl = function(serviceName){
            ...
            .then(function(serviceUrl){
                ...
                $scope.goToStellarTestnetSetting('federation-address');
                vm.initialize();
                vm.getGroups();
                vm.getAllSubtypes();
                vm.getFederationdomainAddresses();
