(function () {
    ...
    /** @ngInject */
    function BitcoinHotwalletCtrl($scope,localStorageManagement,currenciesList,_,$http,errorHandler,toastr,sharedResources,extensionsHelper,$state,
        ...
        $scope.searchHotwalletFees = function(){
            if($scope.feeSubtype){
                $state.go('transactions.history', {searchHotwalletFees: {code: 'XBT', subtype: $scope.feeSubtype} });
            }
            else{
        ...
        $scope.searchHotwalletWithdrawals = function(){            
            if($scope.withdrawSubtype){
                $state.go('transactions.history', {searchHotwalletFees: {code: 'XBT', subtype: $scope.withdrawSubtype, status: 'Pending'} });
            }
            else{
        ...
        $scope.searchUnfundedHotwalletTransactions = function(){
            if($scope.sendSubtype){
                $state.go('transactions.history', {searchHotwalletFees: {code: 'XBT', subtype: $scope.sendSubtype, status: 'Pending'} });
            }
            else{
        ...
        $scope.goToHotWalletPage = function(){
            $state.go('bitcoinServiceAccounts', {view: 'hotwallet'}, {reload: true});
        };
        ...
        $scope.openSendFromHotwalletModal = function (page, size) {
            vm.theModal = $uibModal.open({
                ...
                templateUrl: page,
                size: size,
                controller: 'SendBitcoinHotwalletModalCtrl',
                scope: $scope,
                resolve: {
