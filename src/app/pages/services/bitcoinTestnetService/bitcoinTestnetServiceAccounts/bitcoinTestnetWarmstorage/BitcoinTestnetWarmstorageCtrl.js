(function () {
    ...
    angular.module('BlurAdmin.pages.services.bitcoinTestnetService.bitcoinTestnetServiceAccounts')
        .controller('BitcoinTestnetWarmstorageCtrl', BitcoinTestnetWarmstorageCtrl);

    /** @ngInject */
    function BitcoinTestnetWarmstorageCtrl($scope,localStorageManagement,errorHandler,currenciesList,$http,$uibModal,multiOptionsFilterService,$state,cleanObject,Rehive,
        ...
        $scope.hdKeyValidPrefixes = ["xpub", "upub", "vpub", "Upub", "Vpub", "tpub"];
        ...
        vm.getOnChainBalances = function(warmstorageAccount){
            if(vm.token) {
                $http.get(vm.serviceUrl + 'admin/hdkeys/' + warmstorageAccount.id + '/?expand=onchain_data', {
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': vm.token
                    }
        ...
        $scope.addWarmStoragePublicAddressModal = function (page,size) {
            vm.theModal = $uibModal.open({
                ...
                templateUrl: page,
                size: size,
                controller: 'AddBitcoinTestnetHdKeyCtrl',
                scope: $scope
            });
